...
from Desktop.utils import rmse, mae, sd, pearson
...
def evaluate(model, loader):
    ...
    for batch_data in loader:
        ...
        y_hat = model( a2a_g, Hbond_graphs, hyb_graphs,van_graphs,PAI_graphs)
...
def train(args, model, trn_loader, tst_loader, val_loader):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        running_log += log
        f = open(os.path.join(args.model_dir, 'atom_h3(no_van1).txt'), 'w')
        f.write(running_log)
        f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--stop_epoch', type=int, default=400)
