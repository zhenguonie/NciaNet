...
def process_dataset(core_path, refined_path, dataset_name, output_path, cutoff):
    ...
    txt_path='./Hbond/LIT-PCBA'
    label_data = load_label_data('./data/VS_task_label_dict.pkl')
    data_dict = processed_dict
    for k,v in processed_dict.items():
        v['label'] = label_data[k]
        data_dict[k] = v
    ...
    for k, v in tqdm(data_dict.items()):
        ...
        cofeat, label = v['type_pair'], v['label']
        print(n)
        ...
        if k in core_set_list:
            ...
            core_pk.append(label)
            continue
        refined_id.append(k)
        refined_data.append(graph)
        refined_pk.append(label)
    ...
    print(os.path.join(output_path, dataset_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path_core', type=str, default='./data/test set')
    parser.add_argument('--data_path_refined', type=str, default='./data/training set')
    parser.add_argument('--output_path', type=str, default='./data/')
    parser.add_argument('--dataset_name', type=str, default='LIT-PCBA')
